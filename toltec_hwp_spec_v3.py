...
#if working in jupyter notebooks uncomment the next line
#%matplotlib inline
import csv
...
def hwp_analysis(file_list):
    ...
    for file in file_list:
        ...
        #maybe a sloppy way of doing it but, hey, it gets the job done
        else:
    ...
    plt.savefig('hwp_results_code_thesis.png')
...
with open('model_V1.csv') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    linecount = 0
    for row in csv_reader:
        if linecount<=1:
            ...
            T_C.append(float(row[1]))
            R_C.append(float(row[2]))
            Abs_C.append(float(row[3]))
            T_L.append(float(row[4]))
            R_L.append(float(row[5]))
            Abs_L.append(float(row[6]))
            linecount+=1

plt.figure(1, figsize=(15,10))

plt.plot(frequency, T_C, 'b-', label='C-Axis Transmission')
plt.plot(frequency, T_L, 'r-', label='L-Axis Transmission')
plt.plot(frequency, Abs_C, 'g-', label='C-Axis Absorption')
plt.plot(frequency, Abs_L, 'c-', label='L-Axis Absorption')
plt.plot(frequency, R_C, 'k-', label='C-Axis Reflection')
plt.plot(frequency, R_L, 'm-', label='L-AxisReflection')
xc = [128,170,195,244.5,245.5,310]
for i in range(len(xc)):
        plt.axvline(x=xc[i], color=gray, linestyle='dashed')
plt.title('MHWP V1 data', size='x-large')
plt.xlabel('Frequency (GHz)', size='x-large')
#plt.ylabel()
plt.legend(loc='best', bbox_to_anchor=(0.7, 0., 0.25, 0.4), fontsize='x-large')
plt.text(150,0.5, '150 GHz\nBand', ha='center', size='xx-large')
plt.text(220,0.5, '220 GHz\nBand', ha='center', size='xx-large')
plt.text(280,0.5, '300 GHz\nBand', ha='center', size='xx-large')
plt.xlim(100,320)
plt.grid(b=True)
plt.savefig('HWP_emissivity.png')
...
plt.plot(freq, T_eff)
plt.grid(True)
plt.xlabel('Frequency (GHz)', size='large')
plt.ylabel('Atmosphere Antenna\nTemperature (K)', size='large')
plt.title('25% Atmosphere Antenna\nTemperature At LMT', size='x-large')
plt.ylim(0,100)
...
plt.text(150,50, '150 GHz\nBand', ha='center', size='large')
plt.text(220,50, '220 GHz\nBand', ha='center', size='large')
plt.text(280,50, '300 GHz\nBand', ha='center', size='large')

plt.savefig('atm_antenna_temp.png')
